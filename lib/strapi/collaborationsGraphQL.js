...
export const fetchStrapiCollaboration = async (id, preview = false) => {
  ...
  // get project and assign to a project variable
...
// construct a new project object combining and massaging/flattening data from 
  ...
  const payload = {
      ...
      projects: collaborationData.collaborations.data[0].attributes.projects.data.map((project) => ({
        slug: project.attributes.project_id,
        name: project.attributes.name,
      })),
      partners: collaborationData.collaborations.data[0].attributes.partners.data.map((partner) => ({
        slug: partner.attributes.org_id,
        name: partner.attributes.name,
        orgURL: partner.attributes.url
      })),
      funding: collaborationData.collaborations.data[0].attributes.funding.data.map((partner) => ({
        slug: partner.attributes.org_id,
        name: partner.attributes.name,
        orgURL: partner.attributes.url
      }))
  ...
  // return one mega project-people object
  return payload
}
