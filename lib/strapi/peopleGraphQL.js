...
export const fetchStrapiPerson = async (id) => {
  ...
  const getPayload = (personData) => {
    ...
    return {
      ...
      team: person.teams.data[0]?.attributes ? person.teams.data.map((team) => ({
        name: team.attributes.name,
        slug: team.attributes.slug,
      })) : null,
      researchGroup: person.researchGroups.data[0]?.attributes ? person.researchGroups.data.map((group) => ({
